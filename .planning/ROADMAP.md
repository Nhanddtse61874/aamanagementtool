...
- **M12 — Tag icon presets** — **MERGED to `main`** (`f3adf0f`, 2026-07-20) and **pushed to `origin`**, **Mode A**. **Angular 806 → 814.** .NET 475 / ApiTests 541 unchanged — no C# touched.
  ...
  **Two decisions were escalated rather than assumed:** ⚠️ was excluded because Task List draws its own computed `⚠ Late`/`⚠ At risk` chips and a user tag carrying it would be indistinguishable from an automatic assessment; and 📈/📉 were ambiguous in a way that produces **opposite** assignments, so the reading (arrow = the estimate) was chosen by the user, not inferred.
  ...
  🔴 **Merged without a completed UAT, on explicit user instruction.** `G-1` — do the ten glyphs render as colour emoji rather than tofu — is the one check 814 tests structurally cannot make, and it was never reported back. **⬇️ and ⛰️ are the two at risk.**
