...
## Shipped
- **M12 — Tag icon presets** — **MERGED to `main`** (`f3adf0f`, 2026-07-20) and **pushed to `origin`**, **Mode A**. **Angular 806 → 814.** .NET 475 / ApiTests 541 unchanged — no C# touched.
  Ten preset glyph buttons under the icon field in the Settings tag editor: `🔥 ⏳ ⬇️ 👀 💣 🚫 ❓ ⛰️ 📈 📉`. The problem was an **asymmetry, not a defect** — creating a tag asks for icon, text and colour; colour had the OS picker, text needs no help, and icon had a bare text box you could only use if you knew `Win + .` exists. Presets write the **icon only**; the user still types their own label, and free-text entry is retained, so no existing tag was orphaned.
  **No C#, no schema, no contract, no regen** — `Tags.icon` has been `TEXT NOT NULL` since schema v7 and presets write the same field through the existing `setTagIcon()`. One write path, so a preset and a typed glyph cannot diverge.
  **Two decisions were escalated rather than assumed:** ⚠️ was excluded because Task List draws its own computed `⚠ Late`/`⚠ At risk` chips and a user tag carrying it would be indistinguishable from an automatic assessment; and 📈/📉 were ambiguous in a way that produces **opposite** assignments, so the reading (arrow = the estimate) was chosen by the user, not inferred.
  🔴 **The plan carried four defects and the author caught none of them** — a DOM test with no `fakeAsync`/`tick()` (Plan Checker), its ordering still wrong once added (implementer), an unstubbed spy that printed **4 errors while the suite reported green** (implementer), and a **structurally vacuous** assertion (final review). The last is the same family as M9.2's `grid-state.spec.ts:190`. **Plan Checker paid for itself** — M9.2 skipped it and escaped on luck.
  ✅ **UAT PASSED (user, 2026-07-20)** — the glyphs render. `G-1` — do the ten render as colour emoji rather than tofu — is the one check 814 tests structurally cannot make, because **no test renders a font**; the user's pass is the only evidence for it. ⚠️ Confirmed **on Windows only**; emoji fonts differ per platform and nobody has looked from a phone. The merge preceded the pass, per the 2026-07-19 batching decision — the gap closed after the fact, not before.
  🟠 **Open, undecided:** on the edit path a preset **destroys an existing custom glyph with no undo**. Spec §5.5 declined a deselect toggle using reasoning that only holds when creating.
  Spec: `docs/superpowers/specs/2026-07-20-tag-icon-presets-design.md` · Plan: `docs/superpowers/plans/2026-07-20-M12-tag-icon-presets.md` · UAT: `.planning/M12-UAT.md` · Summary: `.planning/M12-SUMMARY.md`
